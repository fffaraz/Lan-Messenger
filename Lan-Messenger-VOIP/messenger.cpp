...
{
    // _udp.joinMulticastGroup(QHostAddress("224.1.2.3"));
    //_udp.bind(QHostAddress("0.0.0.0"), 2880, QUdpSocket::ReuseAddressHint);
    _udp.bind(QHostAddress::Any, 2880, QUdpSocket::ReuseAddressHint);
...
QHostAddress Messenger::getIPbyID(QString id)
{
    QHostAddress adr;
    for(int i=0; i<_peers.count(); i++)
    {
...
void Messenger::startVoice(QString to)
{
    QHostAddress adr = getIPbyID(to);
    vso->connectToHost(adr);
    vso->setEnabled(true);
    vio->setEnabled(true);
}

void Messenger::stopVoice()
{
